...
Usage()
{
   ...
   echo "  g     Install gitops server on OCP cluster"
   echo "  d     Install cloud-native developmemt toolkit on OCP cluster "
   echo "  h     Print this help"
   echo
}

# Get the options
while getopts ":f:s:n:r:gd" option; do
   case $option in
      h) # display Help
         Usage
         exit 1;;
...
if [[ -n "${GITOPS}" ]]; then
  GITOPS=($(find "${SCRIPT_DIR}/${FLAVOR_DIR}" -type d -maxdepth 1 -name "200-*" | grep "${SCRIPT_DIR}/${FLAVOR_DIR}/" | sed -E "s~${SCRIPT_DIR}/${FLAVOR_DIR}/~~g" | sort))
...
if [[ -n "${DEVTOOLS}" ]]; then
  DEVTOOLS=($(find "${SCRIPT_DIR}/${FLAVOR_DIR}" -type d -maxdepth 1 -name "220-*" | grep "${SCRIPT_DIR}/${FLAVOR_DIR}/" | sed -E "s~${SCRIPT_DIR}/${FLAVOR_DIR}/~~g" | sort))
...
do
  ...
  if [[ "${name}" =~ ^200 ]] && [[ "${name}" != "${GITOPS}" ]]; then
    continue
  fi

  if [[ "${name}" =~ ^220 ]] && [[ "${name}" != "${DEVTOOLS}" ]]; then
    continue
  fi
